...
export async function readGroupsConfig(ctx) {
    const cfg = (await ctx.state.get({ scopeKind: "instance", stateKey: STATE_KEY }));
    return cfg ?? {};
}
/** Pick the most specific config entry for a channel (by id, then name, then "*"). */
function entryFor(cfg, channelId, channelName) {
    const channels = cfg.channels ?? {};
    return channels[channelId] ?? (channelName ? channels[channelName] : undefined) ?? channels["*"];
}
export function resolveGroupPolicy(cfg, channelId, channelName) {
    ...
    if (policy === "disabled")
        ...
        enabled = entry?.allow === true;
    else
        enabled = entry?.allow !== false; // "open": on unless explicitly disallowed
    return {
        enabled,

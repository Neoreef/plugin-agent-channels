...
const channelBots = new Map();
// ─── Human participants ──────────────────────────────────────────────────────
/** Note a human participant observed in a channel message (non-bot senders). */
export function noteHumanParticipant(channelId, sender) {
...
export function recordChannelBot(channelId, bot) {
...
export function getChannelBots(channelId) {
 ...
 */
export function renderRoster(channelId, channelLabel, selfBotName) {
    ...
    for (const h of humans)
        lines.push(`- ${h.name ?? h.id}${h.email ? ` (${h.email})` : ""}`);
    for (const b of bots)
        lines.push(`- ${b.displayName ?? b.botUniqueName} (agent)`);

...
const HONCHO_FLAG_FILE = path.join(env("PAPERCLIP_HOME") ?? path.join(os.homedir(), ".paperclip"), "honcho-mcp.enabled");
export function honchoEnabled() {
    if (process.env.HONCHO_MCP_ENABLED === "0" || process.env.HONCHO_MCP_ENABLED === "false")
        return false;
...
const HONCHO_MCP_TOOLS_FLAG = path.join(env("PAPERCLIP_HOME") ?? path.join(os.homedir(), ".paperclip"), "honcho-mcp-tools.enabled");
export function honchoMcpToolsEnabled() {
    return existsSync(HONCHO_MCP_TOOLS_FLAG);
}
function honchoBaseUrl() { return env("HONCHO_API_URL") ?? "http://127.0.0.1:18820"; }
function honchoMcpRunner() { return env("HONCHO_MCP_RUNNER") ?? "/home/brian/projects/honcho/mcp/runner.ts"; }
function honchoMcpTsx() {
...
async function peerForCliqUser(ctx, channelUserId) {
 ...
 */
export async function resolveHonchoScope(ctx, params) {
    ...
    if (!agentName) {
        try {
            agentName = (await ctx.agents.get(params.agentId, params.companyId))?.name ?? null;
        }
 ...
 */
export async function recordHonchoTurn(ctx, scope, userMessage, agentReply) {
    if (!userMessage.trim() || !agentReply.trim())
        return;
    const base = `${scope.baseUrl}/v3/workspaces/${encodeURIComponent(scope.workspaceId)}`;
    const postJson = (url, body) => ctx.http.fetch(url, {
        method: "POST",
    ...
    try {
        if (scope.groupPeers && scope.groupPeers.length) {
            ...
            for (const p of scope.groupPeers)
                peerMap[p] = { observe_me: true, observe_others: true };
...
function formatRecall(userPeer, data) {
    const parts = [];
 ...
 */
export async function honchoRecall(ctx, scope, prompt) {
    ...
    try {
        ...
        if (prompt.trim()) {
            qs.set("search_query", prompt.slice(0, 200));
            qs.set("search_top_k", "5");
        }
...
export function honchoMcpServers(scope) {
    return {
        honcho: {
...
export function writeHonchoMcpConfig(scope) {
...
export function honchoInstructions(scope) {
    return [
        "## Memory (Honcho)",

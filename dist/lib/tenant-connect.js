...
import { DATA_CENTERS, CLIQ_SCOPES } from "../constants.js";
 ...
 */
export async function resolveAppOAuthClient(ctx) {
    const cfg = (await ctx.config.get());
    if (!cfg.zohoClientId || !cfg.oauthCallbackUrl)
        return null;
    return {
...
export async function getTenantConnectConfig(ctx, companyId) {
    const stored = (await ctx.state.get(serviceStore.scopeKey(companyId, TENANT_CONFIG_KEY)));
    return {
        dataCenter: stored?.dataCenter ?? DEFAULT_TENANT_CONFIG.dataCenter,
        scopes: stored?.scopes ?? DEFAULT_TENANT_CONFIG.scopes,
        enabledChannels: stored?.enabledChannels && stored.enabledChannels.length > 0
...
export async function saveTenantConnectConfig(ctx, companyId, config) {
 ...
 */
export async function ensureTenantService(ctx, companyId, channelType) {
...
async function issueNonce(ctx, companyId, serviceId) {
 ...
 * be replayed.
 */
export async function consumeNonce(ctx, companyId, serviceId, nonce) {
    if (!nonce)
        return false;
    const key = serviceStore.scopeKey(companyId, `${NONCE_KEY_PREFIX}.${nonce}`);
    const rec = (await ctx.state.get(key));
...
export function assembleAuthorizeUrl(opts) {
    ...
    return (`https://${center.accounts}/oauth/v2/auth?` +
        new URLSearchParams({
 ...
 * one code path.
 */
export async function buildTenantConnectUrl(ctx, opts) {
    const { companyId, pluginId } = opts;
    if (!companyId)
        return { configured: false, reason: "Missing companyId." };
    ...
    const state = { serviceId, companyId, channelType, pluginId, src: "tenant", nonce };
 ...
 */
export function renderConnectPage(result) {
    ...
    if (!result.configured) {
        return shell("Connect your org", `<h1>Not ready to connect</h1><p>${escapeHtml(result.reason)}</p>`);
    }
    return shell("Connect your org", `<h1>Connect your workspace</h1>` +
...
//# sourceMappingURL=tenant-connect.js.map